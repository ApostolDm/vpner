...
func NewDNSServer(cfg ServerConfig, ipManager IPSyncer, resolver *doh.Resolver) *DNSServer {
	s := &DNSServer{
		config:        cfg,
...
func (s *DNSServer) handleDNSRequest(w dns.ResponseWriter, r *dns.Msg) {
	...
	defer func() {
		if r := recover(); r != nil {
			log.Printf("panic in DNS handler: %v", r)
		}
	}()
	...
	default:
		...
		log.Println("DNS server overloaded")
	...
	if resolverIP := s.matchCustomResolver(domain); resolverIP != "" {
		...
		if s.config.Verbose {
			log.Printf("DNS response to %s for %s (custom %s, %s): %s",
	...
	if s.config.Verbose {
		log.Printf("DNS response to %s for %s (%s): %s",
