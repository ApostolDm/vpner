...
func (r *XrayRouter) RoutingIntact() bool {
	if !r.ready() {
		return true
	}
...
func (r *XrayRouter) Apply(chain string, info proxy.ChainInfo) error {
	...
	if r.iptables.TProxyEnabled() {
		return r.iptables.BatchApplyAllTProxy([]firewall.ChainSpec{spec})
	}
	return r.iptables.BatchApplyAllRedirect([]firewall.ChainSpec{spec})
}
...
func (r *XrayRouter) Restore(info map[string]proxy.ChainInfo, isRunning func(string) bool, restoreV4, restoreV6 bool, table string) {
	if !r.ready() {
		return
	}
