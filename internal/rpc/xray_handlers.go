...
func (s *VpnerServer) XrayList(_ context.Context, _ *grpcpb.Empty) (*grpcpb.XrayListResponse, error) {
	xrayList, err := s.xrayService.ListInfo()
	if err != nil {
...
func (s *VpnerServer) XraySetAutorun(_ context.Context, req *grpcpb.XrayAutoRunRequest) (*grpcpb.GenericResponse, error) {
	if req.ChainName == "" {
		return errorGeneric("Chain name is required"), nil
	}
	if err := s.xrayService.SetAutorun(req.ChainName, req.AutoRun); err != nil {
...
func (s *VpnerServer) XrayCreate(_ context.Context, req *grpcpb.XrayCreateRequest) (*grpcpb.GenericResponse, error) {
	name, err := s.xrayService.Create(req.Link, req.AutoRun)
	if err != nil {
...
func (s *VpnerServer) XrayTest(_ context.Context, req *grpcpb.XrayRequest) (*grpcpb.GenericResponse, error) {
	report, err := s.xrayService.Test(req.ChainName)
	if err != nil {
		return errorGeneric(fmt.Sprintf("Failed to test %s: %v", req.ChainName, err)), nil
	}
	return successGeneric(report), nil
}
