...
func linkChain(iptablesCmd, table, chain, iface string) (jumpRule, error) {
	...
	if listPreroutingRules(iptablesCmd, table)[needle] {
		logx.Debugf("PREROUTING jump already exists: %s -i %s -j %s", table, iface, chain)
		return jmp, nil
	}
