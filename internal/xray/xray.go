...
func (x *XrayManager) StartXray(ctx context.Context, name string) error {
	...
	cmd.Stdout = logging.NewStreamWriter(prefix, logging.LevelInfo)
	cmd.Stderr = logging.NewStreamWriter(prefix, logging.LevelWarn)
	...
	err = cmd.Wait()
	if err != nil {
		logging.Errorf("[xray-%s] exited with error: %v", name, err)
	} else {
		logging.Infof("[xray-%s] exited cleanly", name)
	}
	return err
}
